...
from align.utils.seq_utils   import get_genetic_code, write_fasta, write_rfasta
...
def trim_sequences(path, seq_path, seqs, trimseq, quiet=True):
    ...
    trimmed = [x for x in seqs if not 'ali' in seqs[x]]
    if not quiet:
...
def merge_mcoffee(path, score, sequences, aa=False):
    """
